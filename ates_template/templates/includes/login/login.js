...
login.bind_events = function () {
	...
	$(".form-login").on("submit", function (event) {
		...
		var args = {};
		args.cmd = "login";
		args.usr = harpiya.utils.xss_sanitise(($("#login_email").val() || "").trim());
		args.pwd = $("#login_password").val();
		if (!args.usr || !args.pwd) {
			harpiya.msgprint('{{ _("Both login and password required") }}');
			return false;
		}
	...
	$(".form-signup").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "harpiya.core.doctype.user.user.sign_up";
		args.email = ($("#signup_email").val() || "").trim();
		args.redirect_to = harpiya.utils.sanitise_redirect(harpiya.utils.get_url_arg("redirect-to"));
		args.full_name = harpiya.utils.xss_sanitise(($("#signup_fullname").val() || "").trim());
	...
	$(".form-forgot").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "harpiya.core.doctype.user.user.reset_password";
	...
	$(".form-login-with-email-link").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "harpiya.www.login.send_login_link";
	...
	var login_handlers = {
		200: function (data) {
			if (data.message == 'Logged In') {
				...
				window.location.href = harpiya.utils.sanitise_redirect(harpiya.utils.get_url_arg("redirect-to")) || data.home_page;
			} else if (data.message == 'Password Reset') {
				window.location.href = harpiya.utils.sanitise_redirect(data.redirect_to);
			} else if (data.message == "No App") {
				login.set_status("{{ _('Success') }}", 'green');
				if (localStorage) {
					var last_visited =
						localStorage.getItem("last_visited")
						|| harpiya.utils.sanitise_redirect(harpiya.utils.get_url_arg("redirect-to"));
				...
				if (cint(data.message[0]) == 0) {
					...
					login.set_status('{{ _("Success") }}', 'green');
					harpiya.msgprint(data.message[1])
				}
...
var verify_token = function (event) {
	$(".form-verify").on("submit", function (eventx) {
		...
		args.tmp_id = harpiya.get_cookie('tmp_id');
		if (!args.otp) {
			harpiya.msgprint('{{ _("Login token required") }}');
			return false;
		}
		login.call(args);
		return false;
